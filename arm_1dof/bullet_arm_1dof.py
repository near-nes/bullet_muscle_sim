...
class BulletArm1Dof:
    ...
    SDF_SEARCH_PATH = "/".join(
        [os.environ.get("SDF_MODELS_DIR", "."), SDF_MODELS_SUBDIR]
    )
    ...
    def InitPybullet(
        self,
        g=[0.0, 0.0, 0.0],
        bullet_connect=pybullet.DIRECT,
        ...
        self._timestep = pybullet.getPhysicsEngineParameters(
    ...
    ) -> arm_1dof.robot_arm_1dof.RobotArm1Dof:
        """Load robot from sdf file"""
        self._skel_arm_id = pybullet.loadURDF(
            fileName=robot_sdf_filename,
            useFixedBase=True,
            physicsClientId=self._server_id,
        )
    ...
    def LoadMuscleRobot(
        self, robot_sdf_filename=URDF_MODEL_FILENAME
    ) -> arm_1dof.robot_arm_muscle_1dof.RobotArmMuscle1Dof:
        ...
        self._skel_arm = arm_1dof.robot_arm_muscle_1dof.RobotArmMuscle1Dof(
            bullet_ctrl=self, bullet_body_id=self._skel_arm_id
        )
    ...
    def Simulate(self, sim_time) -> None:
        ...
        while t < sim_time:
            ...
            if self._skel_muscles:
                ...
                self._skel_muscles.SetJointPos(
                    JOINT_INDEX(0), self._skel_arm.JointPos(0)
                )
                self._skel_muscles.SetJointPos(
                    JOINT_INDEX(1), self._skel_arm.JointPos(1)
                )
                self._skel_muscles.SetJointVel(
                    JOINT_INDEX(0), self._skel_arm.JointVel(0)
                )
                self._skel_muscles.SetJointVel(
                    JOINT_INDEX(1), self._skel_arm.JointVel(1)
                )
                ...
                torques = [
                    self._skel_muscles.GetTorque(JOINT_INDEX(0)),
                    self._skel_muscles.GetTorque(JOINT_INDEX(1)),
                ]
